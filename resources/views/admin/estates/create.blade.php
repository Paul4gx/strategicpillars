...
<form id="estate-form" 
      action="{{ route('admin.estates.store') }}" 
      method="POST" 
      enctype="multipart/form-data" 
      class="form-bacsic-infomation flex gap30 flex-column">
   @csrf
    <x-text-input2 title="Estate Name" name="name" :value="old('name')" required />
    <x-textarea2 title="Description" name="description" :value="old('description')" />
    <div class="row mb-3">
    ...
    <fieldset class="text has-top-title">
        <select name="status" class="" tabindex="2" aria-required="true" required>
            ...
            <option value="developing" @if(old('status') == 'developing') selected @endif>Developing</option>
            <option value="upcoming" @if(old('status') == 'upcoming') selected @endif>Upcoming</option>
        </select>
        <label>Status</label>
        @error('status')<div class="text-danger">{{ $message }}</div>@enderror
    </fieldset>
    <x-textarea2 title="Google Map Embed (iframe)" name="map_embed" :value="old('map_embed')" />
    ...
    <div class="form-group mb-3" style="text-align: right;">
        <button type="submit" class="tf-button-primary">Save Estate</button>
    </div>
</form>
 </div>
@endsection 
